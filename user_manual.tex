...
%This option allows experimenters to route all traffic through a NEAT proxy %\url{https://github.com/NEAT-project/neat}~. To enable this feature set the %parameter \identifier{"neat":1} when scheduling the experiment. In a future %version of the NEAT support, options to chnage policy will be added.
...
For a list of the tools currently installed see the folder \identifier{monroe\_base.docker} in our repositories. This base image has been extended to support layer depending on the kind of experiments the experimenter wants to run. The new monroe\_base tags are as follow:
\begin{itemize}
 \item \textbf{monroe/base} This is for all experiments except the one defined below.
 \item \textbf{monroe/base:web} This tag was specifically designed for experiments that use headless browsing such as Chrome or Firefox.
 \item \textbf{monroe/base:virt} This tag is needed for experiments that need to run in a virtual machine.
\end{itemize}
Some of the packages moved from monroe\_base to monroe\_base:web such as:
\begin{itemize*}
	\item moved Firefox and support packages (xvfb, xauth, libgtk-3-0, libasound2, dbus, libdbus-glib-1-2, libgtk2.0-0, libgtk2.0-common, pyvirtualdisplay, selenium, python-dateutil, geckodriver)
	\item Added support for Firefox (56.0.1) and support packages Google chrome(64.*)
\end{itemize*}
...
xz-utils	&	5.1.1alpha+20120614-2+b3		&	amd64	&	XZ-format compression utilities
	\\
